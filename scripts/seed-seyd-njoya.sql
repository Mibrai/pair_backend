...
INSERT INTO programs (id, user_activity_id, title, description, status, is_public, created_at, updated_at) VALUES
    (v_prog1,  v_ua1,  'Football Academie Yaounde',   'Programme d''entrainement football niveau amateur, 3 séances/semaine.',          'PUBLISHED', TRUE,  NOW() - INTERVAL '8 days', NOW() - INTERVAL '1 day'),
    (v_prog2,  v_ua2,  'Running Matin Cameroun',      'Sorties running matinales en groupe, tous les jours sauf dimanche.',             'PUBLISHED', TRUE,  NOW() - INTERVAL '8 days', NOW() - INTERVAL '2 days'),
    (v_prog3,  v_ua3,  'Basket de Rue 3x3',           'Tournois et entrainements basket en plein air, accès libre.',                    'PUBLISHED', TRUE,  NOW() - INTERVAL '8 days', NOW() - INTERVAL '1 day'),
    (v_prog4,  v_ua4,  'Natation Lac Municipal',      'Programme de natation en lac, encadrement professionnel inclus.',                'PUBLISHED', FALSE, NOW() - INTERVAL '7 days', NOW() - INTERVAL '3 days'),
    (v_prog5,  v_ua5,  'Circuit Training Outdoor',    'Renforcement musculaire fonctionnel en plein air, 45 min par séance.',           'PUBLISHED', TRUE,  NOW() - INTERVAL '7 days', NOW() - INTERVAL '2 days'),
    (v_prog6,  v_ua6,  'Yoga Naturel Yaounde',        'Yoga en plein air avec vue sur le Mont Febe, niveaux débutants et confirmés.',   'PUBLISHED', TRUE,  NOW() - INTERVAL '6 days', NOW() - INTERVAL '1 day'),
    (v_prog7,  v_ua7,  'Boxe Technique Initiation',   'Cours de boxe anglaise technique, sans sparring pour débutants complets.',       'PUBLISHED', TRUE,  NOW() - INTERVAL '6 days', NOW() - INTERVAL '2 days'),
    ...
    (v_prog9,  v_ua9,  'Volley Plage Weekend',        'Volley-ball sur terrain extérieur, ambiance détendue, tous les samedis.',        'PUBLISHED', TRUE,  NOW() - INTERVAL '5 days', NOW() - INTERVAL '1 day'),
    (v_prog10, v_ua10, 'MMA Initiation Cameroun',     'Introduction aux arts martiaux mixtes, accent sur la technique et la sécurité.', 'PUBLISHED', TRUE,  NOW() - INTERVAL '4 days', NOW() - INTERVAL '1 day')
ON CONFLICT (id) DO NOTHING;
...
INSERT INTO schedules (id, program_id, place_name, place_type, location, address_public, show_exact_address, starts_at, ends_at, recurrence_rule, max_participants, created_at) VALUES
    (v_sched1,  v_prog1,  'Stade Ahmadou Ahidjo',        'OUTDOOR', ST_SetSRID(ST_MakePoint(11.5102, 3.8700), 4326), 'Avenue Monseigneur Vogt, Yaounde', FALSE, NOW() + INTERVAL '1 day',  NOW() + INTERVAL '1 day 2 hours',             'FREQ=WEEKLY;BYDAY=MO,WE,FR',         20, NOW() - INTERVAL '8 days'),
    (v_sched2,  v_prog2,  'Parc du Mont Febe',           'OUTDOOR', ST_SetSRID(ST_MakePoint(11.5200, 3.8980), 4326), 'Mont Febe, Yaounde',               FALSE, NOW() + INTERVAL '1 day',  NOW() + INTERVAL '1 day 1 hour',              'FREQ=DAILY;BYDAY=MO,TU,WE,TH,FR,SA', 15, NOW() - INTERVAL '8 days'),
    (v_sched3,  v_prog3,  'Terrain Omnisports Essos',    'OUTDOOR', ST_SetSRID(ST_MakePoint(11.5300, 3.8550), 4326), 'Quartier Essos, Yaounde',          TRUE,  NOW() + INTERVAL '2 days', NOW() + INTERVAL '2 days 2 hours',            'FREQ=WEEKLY;BYDAY=TU,TH',            18, NOW() - INTERVAL '8 days'),
    (v_sched4,  v_prog4,  'Lac Municipal de Yaounde',    'OUTDOOR', ST_SetSRID(ST_MakePoint(11.5150, 3.8620), 4326), 'Lac Municipal, Yaounde',           FALSE, NOW() + INTERVAL '3 days', NOW() + INTERVAL '3 days 90 minutes',         'FREQ=WEEKLY;BYDAY=SA',               10, NOW() - INTERVAL '7 days'),
    (v_sched5,  v_prog5,  'Parc de la Reunification',   'OUTDOOR', ST_SetSRID(ST_MakePoint(11.5080, 3.8450), 4326), 'Avenue Kennedy, Yaounde',          FALSE, NOW() + INTERVAL '1 day',  NOW() + INTERVAL '1 day 45 minutes',          'FREQ=WEEKLY;BYDAY=MO,WE,FR',         25, NOW() - INTERVAL '7 days'),
    (v_sched6,  v_prog6,  'Jardin Botanique de Yaounde', 'OUTDOOR', ST_SetSRID(ST_MakePoint(11.5010, 3.8500), 4326), 'Jardin Botanique, Yaounde',        FALSE, NOW() + INTERVAL '2 days', NOW() + INTERVAL '2 days 90 minutes',         'FREQ=WEEKLY;BYDAY=TU,TH,SA',         12, NOW() - INTERVAL '6 days'),
    (v_sched7,  v_prog7,  'Salle Polyvalente Omnisports','INDOOR',  ST_SetSRID(ST_MakePoint(11.5180, 3.8610), 4326), 'Complexe Omnisports, Yaounde',     TRUE,  NOW() + INTERVAL '1 day',  NOW() + INTERVAL '1 day 90 minutes',          'FREQ=WEEKLY;BYDAY=MO,WE',            16, NOW() - INTERVAL '6 days'),
    (v_sched8,  v_prog8,  'Circuit Collines de Yaounde', 'OUTDOOR', ST_SetSRID(ST_MakePoint(11.5250, 3.8750), 4326), 'Point de depart: Nlongkak',        FALSE, NOW() + INTERVAL '4 days', NOW() + INTERVAL '4 days 3 hours',            'FREQ=WEEKLY;BYDAY=SU',                8, NOW() - INTERVAL '5 days'),
    (v_sched9,  v_prog9,  'Beach-Volley Club Yaounde',   'OUTDOOR', ST_SetSRID(ST_MakePoint(11.5060, 3.8430), 4326), 'Quartier Bastos, Yaounde',         FALSE, NOW() + INTERVAL '5 days', NOW() + INTERVAL '5 days 2 hours',            'FREQ=WEEKLY;BYDAY=SA',               16, NOW() - INTERVAL '5 days'),
    (v_sched10, v_prog10, 'Dojo Centre Sportif',         'INDOOR',  ST_SetSRID(ST_MakePoint(11.5130, 3.8560), 4326), 'Centre Sportif, Yaounde',          TRUE,  NOW() + INTERVAL '2 days', NOW() + INTERVAL '2 days 90 minutes',         'FREQ=WEEKLY;BYDAY=TU,FR',            14, NOW() - INTERVAL '4 days')
ON CONFLICT (id) DO NOTHING;
