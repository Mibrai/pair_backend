...
BEGIN
  ...
  -- TEMP TABLES
  -- =========================================================
  CREATE TEMP TABLE _uas   (u_idx INT, seq INT, ua_id UUID, uid UUID)    ON COMMIT DROP;
  CREATE TEMP TABLE _progs (u_idx INT, seq INT, prog_id UUID, uid UUID)  ON COMMIT DROP;
  ...
  FOR i IN 1..5 LOOP
    ...
    FOR rec IN
      SELECT id
      FROM   activities
      WHERE  id NOT IN (SELECT activity_id FROM user_activities WHERE user_id = uid)
      ORDER  BY created_at
      LIMIT  GREATEST(0, 20 - (SELECT COUNT(*) FROM user_activities WHERE user_id = uid)::INT)
    LOOP
      cur_id := gen_random_uuid();
      INSERT INTO user_activities(id, user_id, activity_id, visible_on_map, level, format, created_at)
      VALUES(cur_id, uid, rec.id, j%3 != 2,
      ...
      INSERT INTO _uas VALUES(i, j, cur_id, uid);
      j := j + 1;
    END LOOP;
    -- Store existing user_activities too (for program distribution)
    FOR rec IN SELECT id FROM user_activities WHERE user_id = uid AND id NOT IN (SELECT ua_id FROM _uas) ORDER BY created_at LOOP
      INSERT INTO _uas VALUES(i, j + 100, rec.id, uid);
      j := j + 1;
    END LOOP;
  END LOOP;
  ...
  FOR i IN 1..5 LOOP
    ...
    FOR j IN 0..19 LOOP
      ...
      INSERT INTO _progs VALUES(i, j, cur_id, uid);
    END LOOP;
  END LOOP;
  ...
  FOR i IN 1..5 LOOP
    ...
    LOOP
      ...
      WHERE (ua = uid AND ub = rec.owner_id) OR (ub = uid AND ua = rec.owner_id)
      LIMIT 1;
      -- Fallback
      ...
      VALUES(cur_id, rec.prog_id, uid, cur_conv,
        (3.0 + (j % 5) * 0.5)::FLOAT,
        (ARRAY[
  ...
  FROM (VALUES (u1,'Alice'),(u2,'Bob'),(u3,'Claire'),(u4,'David'),(u5,'Emma')) AS u(uid, uname)
  ...
  WHERE NOT EXISTS (
    SELECT 1 FROM notification_prefs np WHERE np.user_id = uid AND np.notification_type = t.ntype
  );
  ...
  FOR i IN 1..5 LOOP
    ...
    LOOP
      IF NOT EXISTS (SELECT 1 FROM reports WHERE reporter_id = uid AND target_id = rec.prog_id) THEN
        INSERT INTO reports(id, reporter_id, target_type, target_id, reason, status, created_at, resolved_at)
        VALUES(gen_random_uuid(), uid, 'PROGRAM', rec.prog_id,
  ...
  SELECT
    gen_random_uuid(), u.uid,
    'fcm_' || u.uname || '_v2_device' || n::TEXT || '_' || LEFT(MD5(u.uid::TEXT || n::TEXT), 16),
